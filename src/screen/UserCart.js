...
const UserCart = ({ navigation }) => {
    ...
    useEffect(() => {
        if (cartdata != null) {
            ...
            foodprice.map((item) => {
                if (item.data) { // Ensure item.data is defined
                    totalfoodprice += (parseInt(item.data.foodPrice) * parseInt(item.Foodquantity || 1)) +
                        (parseInt(item.data.foodAddonPrice || 0) * parseInt(item.Addonquantity || 0));
                }
            });
    ...
    renderItem={({ item }) => {
        return (
            item.data ? (
                ...
                <View style={styles.cartcard}>
                    <Text style={styles.txt1}>Item data is missing or incomplete</Text>
                </View>
            )
        );
    }}
